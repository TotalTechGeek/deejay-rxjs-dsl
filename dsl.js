    ...
    for (const operators of priorities) {
        ...
        while(arr.length) {
            ...
            while(prevIndex >= 0 && /[$@.A-Za-z0-9_()^,'"#]/.exec(str[prevIndex-1])) {
                if (str[prevIndex - 1] === ')')  parenthCount++
                else if (str[prevIndex - 1] === '(')  parenthCount--
                else if (str[prevIndex - 1] === ',') {
            ...
            while(nextIndex < str.length +1 && /[$@.A-Za-z0-9_()^,'"#]/.exec(str[nextIndex + 1])) {
                if (str[nextIndex + 1] === '(')  parenthCount++
                else if (str[nextIndex + 1] === ')')  parenthCount--
 ...
 * @returns {Function} 
 */
function buildOperator (name, logic, { n = 1, eval = false, extra = [] } = {}) {
    ...
    if (n === 1) {
        if (accumulators.has(name)) {
            ...
            return operator(
                (...args) => f({ accumulator: args[0], current: args[1] }), 
                ...extra
            )
        } 

        let f = engine.build(logic)
        if (eval || fixedOperators.has(name)) f = f()
